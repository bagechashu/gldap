...
func NewDatabaseHandler(sqlBackend SqlBackend, opts ...Option) Handler {
...
func (h databaseHandler) FindUser(userName string, searchByUPN bool) (f bool, u config.User, err error) {
	...
	err = h.database.cnx.QueryRow(fmt.Sprintf(`
			SELECT u.uidnumber,u.primarygroup,u.passbcrypt,u.passsha256,u.otpsecret,u.disabled
			FROM users u WHERE %s=%s`,
		criterion,
		h.sqlBackend.GetPrepareSymbol()), userName).Scan(
		&user.UIDNumber, &user.PrimaryGroup, &user.PassBcrypt, &user.PassSHA256, &user.OTPSecret, &disabled)
	if err == nil {
...
func (h databaseHandler) FindPosixAccounts(hierarchy string) (entrylist []*ldap.Entry, err error) {
	...
	rows, err := h.database.cnx.Query(`
		SELECT u.name,u.uidnumber,u.primarygroup,u.passbcrypt,u.passsha256,u.otpsecret,u.othergroups,u.givenname,u.sn,u.mail,u.loginshell,u.homedirectory,u.disabled,u.sshkeys,u.custattr  
		FROM users u`)
	if err != nil {
		return entries, err
	}
	...
	u := config.User{}
	for rows.Next() {
		err := rows.Scan(&u.Name, &u.UIDNumber, &u.PrimaryGroup, &u.PassBcrypt, &u.PassSHA256, &u.OTPSecret, &otherGroups, &u.GivenName, &u.SN, &u.Mail, &u.LoginShell, &u.Homedir, &disabled, &sshKeys, &custattrstr)
		if err != nil {
			return entries, err
		}
...
func (h databaseHandler) getGroupMemberDNs(gid int) []string {
	...
	rows, err := h.database.cnx.Query(`
			SELECT u.name,u.uidnumber,u.primarygroup,u.passbcrypt,u.passsha256,u.otpsecret,u.othergroups
			FROM users u`)
	if err != nil {
		// Silent fail... for now
		return []string{}
	...
	u := config.User{}
	for rows.Next() {
		err := rows.Scan(&u.Name, &u.UIDNumber, &u.PrimaryGroup, &u.PassBcrypt, &u.PassSHA256, &u.OTPSecret, &otherGroups)
		if err != nil {
			return []string{}
		}
